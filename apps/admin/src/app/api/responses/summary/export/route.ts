import { exportClubSummaryCsvFromFilters } from '@quizmania/shared';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const csvData = await exportClubSummaryCsvFromFilters(filters);
    const filename = `quizmania-club-summary-${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    console.error('Error exporting club summary CSV:', err);
    return new Response('Failed to export club summary CSV', { status: 500 });
  }
}
