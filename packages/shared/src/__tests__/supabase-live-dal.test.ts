...
async function runLiveDalTests() {
  ...
  try {
    await test('1. Ready & Database Status', async () => {
      ...
      assert(isReady === true, 'isSupabaseDatabaseReady returns true with mock client');

      const dbStatus = await getDatabaseStatus();
      assert(dbStatus.ready === true && dbStatus.mode === 'supabase', 'getDatabaseStatus reports supabase mode when ready');

      const authClient = getSupabaseAuthenticatedClient('live-test-token');
      assert(authClient !== null, 'getSupabaseAuthenticatedClient instantiates client');
    });

    await test('2. Storage with live client', async () => {
      const uploaded = await uploadImage('quiz-covers', new Blob(['fake']), 'cover.png');
      assert(uploaded.success && Boolean(uploaded.url), 'uploadImage succeeds via live storage client');

      const publicUrl = getStorageImageUrl('quiz-covers', 'quiz-covers/cover.png');
      assert(publicUrl.includes('https://storage.supabase.co'), 'getStorageImageUrl returns public URL via client');
    });

    await test('3. DAL Admin: Live queries', async () => {
      const liveQuizzes = await getAllQuizzes();
      assert(liveQuizzes.length > 0, 'getAllQuizzes fetches from live table');

      const liveQuiz = await getQuizById(LIVE_QUIZ_ID);
      assert(liveQuiz !== null && liveQuiz.title === 'Live Supabase Test Quiz', 'getQuizById fetches live quiz');

      const liveSlugQuiz = await getQuizBySlug('live-supabase-test-quiz');
      assert(liveSlugQuiz !== null, 'getQuizBySlug fetches live quiz');
      ...
      assert(updatedQuiz !== null && updatedQuiz.title === 'Updated Live Quiz', 'saveQuiz updates live quiz record');

      const livePaginated = await getResponsesPaginated({ quizId: LIVE_QUIZ_ID });
      assert(livePaginated.items.length >= 0, 'getResponsesPaginated runs live query');

      const liveDetail = await getResponseDetail('sub-live-1');
      assert(liveDetail !== null && liveDetail.submission.id === 'sub-live-1', 'getResponseDetail fetches live submission with answers');

      const liveGrade = await updateManualGrade('sub-live-1', LIVE_Q_ID, 4);
      assert(liveGrade.success, 'updateManualGrade updates answer marks in live table');

      const liveAdmin = await getAdminUserRecord('live-admin@quizmania.dev');
      assert(liveAdmin !== null && liveAdmin.role === 'admin', 'getAdminUserRecord finds user in live table');
      ...
      assert(liveSavedAdmin !== null, 'saveAdminUserRecord persists user to live table');
    });

    await test('4. DAL Public: Live queries', async () => {
      const livePubList = await getPublishedQuizzesList();
      assert(Array.isArray(livePubList), 'getPublishedQuizzesList runs live query');
      ...
      assert(livePubQuiz !== null, 'getPublishedQuizBySlug returns published quiz object');
      assert(livePubQuiz?.title === 'Updated Live Quiz', 'getPublishedQuizBySlug has updated quiz title');
      assert(Array.isArray(livePubQuiz?.questions) && livePubQuiz?.questions.length > 0, 'getPublishedQuizBySlug has questions');
      assert(Array.isArray(livePubQuiz?.sections) && livePubQuiz?.sections.length > 0, 'getPublishedQuizBySlug has sections');
      assert(livePubQuiz?.totalMarks > 0, 'getPublishedQuizBySlug calculates totalMarks');
      assert(livePubQuiz?.availability?.isAvailable === true, 'getPublishedQuizBySlug sets availability');
      ...
      assert(liveAtt.session_token === 'tok_live_new', 'createQuizAttempt saves to live table');

      const fetchedLiveAtt = await getQuizAttemptByToken('live_tok_123');
      assert(fetchedLiveAtt !== null, 'getQuizAttemptByToken fetches from live table');

      await updateQuizAttemptStatus('live_tok_123', 'completed');
      assert(true, 'updateQuizAttemptStatus updates live attempt record');

      // Now test setQuizStatus transition to draft
      const liveStatusUpdate = await setQuizStatus(LIVE_QUIZ_ID, 'draft');
      assert(liveStatusUpdate?.status === 'draft', 'setQuizStatus updates status on live quiz');
    });
    ...
    await test('5. Live Supabase Auth Guard', async () => {
      ...
      assert(liveAuthRes.authorized, 'requireAuthenticatedAdmin authenticates via live Supabase Auth');

      const liveDeleted = await deleteQuiz(LIVE_QUIZ_ID);
      assert(liveDeleted, 'deleteQuiz removes live quiz');
    });
  } finally {
    resetSupabaseClients();
  }
